 ...
 module.exports = function($, _) {
  ...
  function ready() {
    ...
    $.ajax({
      ...
      function(result){           
            ...
            for (var i = 0; i < story[0]['slide'].length; i++) {
              var block_id = i + 1; 
              if (block_id == 1) {
                buttons += '<a class="button" id="button_' + block_id +'"> <strong>' + story[on_slide]['slide'][i]['text'] +'</strong></a>';
              } else {
                buttons += '<a class="button-completed" id="button_' + block_id +'"> <strong>' + story[on_slide]['slide'][i]['text'] +'</strong></a>';
              }
            }
    ...
    $doc.on('click', 'a', function(e) {
      if (e.currentTarget.id == "next_button") {
        on_slide += 1;
        document.getElementById('image_container').innerHTML = '';
        document.getElementById('displayed_text').innerHTML = '';
        var buttons = "";
      ...
      } else if (e.currentTarget.id == "restart_button") {
        on_slide = 0;
        document.getElementById('image_container').innerHTML = '';
        document.getElementById('displayed_text').innerHTML = '';
        var buttons = "";
        ...
        // only say blocks if clicked in order
        if (e.currentTarget.className == "button-selected") {
          ...
          for (var i = 0; i < story[on_slide]['slide'].length; i++) {
            var block_id = i + 1;
            if (document.getElementById('button_' + block_id).className == "button") {  
              block_found = true;
              document.getElementById('button_' + block_id).className = "button-selected";
              if (block_id < story[on_slide]['slide'].length) {
              ...
              if (i == story[on_slide]['slide'].length - 1) {
                ...
                if (on_slide == story.length - 1) {
                  // last slide
                  document.getElementById('button_container').innerHTML +=  '<a class="button" id="restart_button"> <strong>&lt;</strong></a>';
                } else {
                  document.getElementById('button_container').innerHTML +=  '<a class="button" id="next_button"> <strong>&gt;</strong></a>';
                }
              }
              break;
            }
          ...
          if (on_last_block) {
            for (var i = 0; i < story[on_slide]['slide'].length; i++) {
              var block_id = i + 1;
              document.getElementById('button_' + block_id).className = "button-completed";
            }  
          }
        }
        if (!block_found) {
